...
import { 
  ...
  AlertCircle, Package, Menu, UserSquare, Users, Bell, User
} from 'lucide-react'

// 1. Importação dos Componentes (Note que agora incluímos o PerfilTab)
...
// 2. Definição das Abas (Adicionamos 'perfil' aqui)
    ...
    <div className="min-h-screen bg-[#f5f3ef] flex flex-col relative font-sans selection:bg-pink-200 overflow-x-hidden">
      ...
      <header className="relative z-10 flex justify-between items-center px-6 pt-8 pb-4">
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => setIsNotificacoesOpen(true)} className="relative w-10 h-10 rounded-full bg-white/10 backdrop-blur-md flex items-center justify-center text-white border border-white/20 shadow-lg">
            <Bell size={18} />
          ...
          {/* 3. Botão de Perfil: Agora chama a função changeTab('perfil') */}
          <button
      ...
      {/* 4. Renderização Condicional: O que aparece no meio da tela depende do valor de 'tab' */}
      ...
      {isMenuOpen && (
        <>
          ...
          <div className="fixed bottom-24 right-4 z-50 bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden w-48 py-2 animate-in slide-in-from-bottom-4">
            ...
            {/* 5. Opção do Perfil dentro do menu lateral também */}
