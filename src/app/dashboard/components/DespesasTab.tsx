...
export default function DespesasTab() {
  ...
  return (
    <div className="px-4 py-2 flex flex-col gap-6">
      ...
      {/* Faixa de KPIs (Sem margem negativa para não colar no header) */}
      ...
      <div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col gap-3">
            {custos.map(custo => (
              <div key={custo.id} className="bg-white rounded-[20px] p-4 shadow-[0_2px_16px_rgba(0,0,0,0.04)] border border-slate-100 flex items-center justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <p className="font-bold text-slate-800 truncate leading-snug">{custo.descricao}</p>
                  <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                    <span className="text-[10px] font-bold bg-slate-100 text-slate-600 px-2.5 py-0.5 rounded-full">
                      {custo.categoria}
                    </span>
                    <span className="text-[10px] font-medium text-slate-400">{formatarData(custo.data_custo)}</span>
                  </div>
                </div>
                <div className="flex flex-col items-end gap-1.5 shrink-0">
                  ...
                  <button
                    ...
                    className="text-[10px] font-bold text-slate-300 hover:text-red-500 flex items-center gap-1 transition-colors"
                  >
                    {excluindo === custo.id
      ...
      {/* MODAL BOTTOM SHEET: Nova Despesa (Blindado no Z-Index 60) */}
      {modalAberto && (
        <div className="fixed inset-0 z-[60] flex items-end justify-center sm:items-center">
          ...
          <div className="relative bg-white w-full max-w-lg rounded-t-[32px] sm:rounded-[32px] p-6 pb-32 shadow-2xl animate-in slide-in-from-bottom-full duration-300 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
              <div>
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 flex items-center gap-1.5"><Calendar size={14} /> Data</label>
                <input type="date" {...register('data_custo', { required: true })} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3.5 text-base focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 transition-all" />
              </div>

              <div>
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 flex items-center gap-1.5"><Tag size={14} /> Categoria</label>
                <select {...register('categoria', { required: true })} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3.5 text-sm focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 transition-all">
              ...
              <div>
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 flex items-center gap-1.5"><FileText size={14} /> Descrição</label>
                <input type="text" placeholder="Ex: 2 barras de parafina" {...register('descricao', { required: true })} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3.5 text-base focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 transition-all" />
              </div>

              <div>
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 flex items-center gap-1.5"><DollarSign size={14} /> Valor (R$)</label>
                <input type="number" step="0.01" min="0" placeholder="0,00" {...register('valor_custo', { required: true, valueAsNumber: true })} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3.5 text-base focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 transition-all" />
              </div>

              <button
                type="submit"
                disabled={salvando}
                className="w-full bg-gradient-to-br from-red-500 to-rose-600 text-white font-bold py-5 rounded-xl text-lg mt-4 shadow-[0_4px_14px_rgba(239,68,68,0.4)] active:scale-[0.98] transition-transform disabled:opacity-60 flex items-center justify-center gap-2"
              >
