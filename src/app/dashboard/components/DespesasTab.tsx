...
export default function DespesasTab() {
  ...
  // Traduz as categorias para o seletor visual, mas salva em português no banco para manter o histórico
  const CATEGORIAS_MAP: Record<string, string> = {
    'Manutenção': t.despesasTab.manutencao,
    'Equipamentos': t.despesasTab.equipamentos,
    'Marketing': t.despesasTab.marketing,
    'Outros': t.financeiroTab.outros
  }
  ...
  async function handleAddDespesa(e: React.FormEvent) {
    ...
    if (error) {
      alert(t.despesasTab.erroSalvar + error.message)
    } else {
      setDescricao('')
      setValor('')
  ...
  async function excluirDespesa(id: string) {
    if (!window.confirm(t.despesasTab.confirmaArquivar)) return
    ...
    <div className="px-4 py-2 flex flex-col gap-6 animate-in fade-in duration-500 relative min-h-[80vh]">
      ...
      <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-[28px] p-6 shadow-xl relative overflow-hidden">
        <div className="relative z-10">
          <span className="text-[10px] font-black text-white/50 uppercase tracking-[0.2em]">{t.despesasTab.totalGastos}</span>
          ...
          <div className="mt-4 flex items-center gap-2">
            <span className="bg-rose-500/20 text-rose-300 text-[10px] font-bold px-3 py-1 rounded-full border border-rose-500/30 flex items-center gap-1">
              <Receipt size={10} /> {despesas.length} {t.despesasTab.registros}
            </span>
          </div>
        </div>
      ...
      <div className="flex flex-col gap-3 pb-24">
        <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">{t.despesasTab.historicoSaidas}</h3>
        ...
        {loading ? (
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter flex items-center gap-1">
                      <Calendar size={10} /> {new Date(item.data_despesa).toLocaleDateString()}
                    </span>
          ...
          <div className="bg-white/50 border-2 border-dashed border-slate-200 rounded-[32px] py-12 flex flex-col items-center justify-center text-center px-6">
            ...
            <p className="text-slate-500 font-bold">{t.despesasTab.nenhumGasto}</p>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 animate-in fade-in zoom-in duration-200">
          ...
          <form onSubmit={handleAddDespesa} className="relative w-full max-w-md bg-white rounded-[32px] p-6 shadow-2xl flex flex-col gap-5">
            <div className="flex justify-between items-center border-b border-slate-50 pb-4">
              <h3 className="text-lg font-black text-slate-800 tracking-tight">{t.despesasTab.registrarGasto}</h3>
              <button type="button" onClick={() => setIsModalOpen(false)} className="text-slate-300 hover:text-slate-500"><X size={24} /></button>
            </div>

            <div className="space-y-4">
              <div>
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-1 block">{t.despesasTab.oQueComprou}</label>
                <input required type="text" placeholder={t.despesasTab.exCompra} value={descricao} onChange={e => setDescricao(e.target.value)} className="w-full bg-slate-50 border-slate-100 rounded-2xl px-4 py-4 focus:ring-2 focus:ring-pink-500 outline-none font-bold text-slate-700" />
              </div>
              
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-1 block">{t.despesasTab.valor}</label>
                ...
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-1 block">{t.despesasTab.data}</label>
              ...
              <div>
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-1 block">{t.despesasTab.categoria}</label>
                <select value={categoria} onChange={e => setCategoria(e.target.value)} className="w-full bg-slate-50 border-slate-100 rounded-2xl px-4 py-4 focus:ring-2 focus:ring-pink-500 outline-none font-bold text-slate-700">
                  {Object.keys(CATEGORIAS_MAP).map(cat => (
                    <option key={cat} value={cat}>{CATEGORIAS_MAP[cat]}</option>
                  ))}
                </select>
              </div>
            </div>
            ...
            >
              {salvando ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <><CheckCircle size={20} /> {t.despesasTab.confirmarSaida}</>}
            </button>
          </form>
        </div>
      )}
