      ...
      <div key={aula.id} className={`bg-white rounded-[24px] p-5 shadow-sm border transition-all ${urgenciaProf ? 'border-red-200' : 'border-slate-100'}`}>
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${aula.modalidade === 'Aula Particular' ? 'bg-pink-100 text-pink-700' : 'bg-violet-100 text-violet-700'}`}>
              {aula.modalidade === 'Aula Particular' ? t.aulas.aulaParticular : t.aulas.aulaGrupo}
            </span>
          </div>
          <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full ${aula.status_pagamento === 'Pago' ? 'bg-emerald-100 text-emerald-700' : aula.status_pagamento === 'Parcial' ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-600'}`}>
            {aula.status_pagamento === 'Pago' ? `✓ ${t.aulas.pagoTotal}` : aula.status_pagamento === 'Parcial' ? t.aulas.parcial : t.aulas.pendente}
          </span>
        </div>
        ...
        {expandido && (
          <div className="mt-4 pt-4 border-t border-slate-100">
            <div className="flex items-center justify-between mb-3">
              <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">{t.aulas.professores}</span>
              {!editandoProfId && <button onClick={() => { setProfsTemporarios(arrayProfs); setEditandoProfId(aula.id); }} className="text-[10px] font-bold text-pink-600 uppercase tracking-wider hover:underline">{t.aulas.alterar}</button>}
            </div>
            ...
            {editandoProfId === aula.id ? (
              <div className="mb-4 bg-pink-50/50 p-3 rounded-2xl border border-pink-100">
                ...
                <div className="flex gap-2">
                  <button onClick={() => setEditandoProfId(null)} className="flex-1 py-2.5 text-xs font-bold text-slate-500 hover:bg-slate-100 rounded-xl transition-colors">{t.aulas.cancelar}</button>
                  <button onClick={() => salvarEdicaoProfessores(aula.id)} className="flex-1 bg-pink-600 text-white text-xs font-bold py-2.5 rounded-xl shadow-sm transition-transform active:scale-95">{t.aulas.confirmar}</button>
                </div>
              </div>
            ) : (
              <div className="grid grid-cols-2 gap-2 mb-4">
                {semProfessor ? (
                  <div className="col-span-2 py-3 bg-slate-50 border border-dashed border-slate-300 rounded-xl text-center text-xs font-bold text-slate-400">{t.aulas.aDefinir}</div>
                ) : (
                  arrayProfs.map(prof => <div key={prof} className="bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs font-bold text-slate-700">{prof.split(' ')[0]}</div>)
                )}
              </div>
            )}
            ...
            <div className="bg-slate-50 rounded-xl p-3 mb-4 space-y-2">
              ...
              {aula.observacoes && (
                <div className="pt-2 border-t border-slate-200 mt-2">
                  <span className="text-[11px] font-bold text-slate-400 uppercase block mb-1">{t.aulas.notas}</span>
                  <p className="text-xs text-slate-600 italic leading-snug">{aula.observacoes}</p>
                </div>
              )}
            </div>

            <button onClick={() => excluirAula(aula)} className="w-full bg-red-50 border border-red-100 text-red-600 font-bold text-xs py-2.5 rounded-xl flex items-center justify-center gap-1.5 hover:bg-red-100 transition-colors"><Trash2 size={14} /> {t.aulas.arquivarAula}</button>
          </div>
        )}
      </div>
    )
    ...
    <>
      <div className="px-4 py-2 flex flex-col gap-6 w-full overflow-x-hidden">
        <div className="flex gap-3 -mt-2">
          <div className="flex-[1.2] bg-gradient-to-br from-pink-500 to-rose-600 rounded-[20px] p-4 shadow-[0_4px_20px_rgba(232,67,106,0.3)] relative overflow-hidden">
            <span className="text-[32px] font-black text-white leading-none">{aulasHoje.length}</span>
            <span className="text-[11px] font-medium text-white/80 mt-1 block">{t.aulas.aulasHoje}</span>
          ...
          <div className="flex-1 flex flex-col gap-2.5">
            <div className="flex-1 bg-white rounded-[16px] p-3 shadow-sm border border-slate-100">
              ...
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{t.aulas.recebido}</span>
            </div>
            <div className="flex-1 bg-white rounded-[16px] p-3 shadow-sm border border-slate-100">
              <span className="text-[18px] font-bold text-slate-800 leading-tight block">{formatarValor(totalDia)}</span>
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{t.aulas.totalDia}</span>
            </div>
          </div>
        </div>
            ...
            HORARIOS_VISIVEIS.length === 0 ? (
              ...
              <div className="bg-white rounded-[24px] shadow-sm border border-slate-100 overflow-hidden flex flex-col mb-24 animate-in fade-in duration-300">
                <div className="p-3 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                  <h3 className="font-bold text-slate-700 text-[13px]">{t.aulas.proximosDias}</h3>
                  <span className="text-[9px] text-slate-400 font-black uppercase tracking-widest bg-white px-2 py-1 rounded-md border border-slate-200">{t.aulas.arraste}</span>
                </div>
                      ...
                      return (
                        <div key={dataStr} className={`flex-1 border-r border-slate-100 relative ${index === 0 ? 'bg-pink-50/30' : ''}`}>
                          <div className={`h-12 border-b border-slate-100 flex flex-col items-center justify-center ${index === 0 ? 'bg-pink-50' : 'bg-white'}`}>
                            <span className={`text-[9px] font-black uppercase tracking-widest ${index === 0 ? 'text-pink-500' : 'text-slate-400'}`}>{index === 0 ? t.aulas.hoje : nomesDias[dia.getDay()]}</span>
                            ...
                            return (
                              <div key={hora} className="h-12 border-b border-slate-100/50 p-0.5 flex gap-0.5 relative">
                                {aulasNesteHorario.map(aulaAqui => (
                                  <div key={aulaAqui.id} onClick={() => { setAulaDetalheGrade(aulaAqui); setCardExpandido(aulaAqui.id); }} className={`flex-1 rounded-[6px] p-1 cursor-pointer border flex flex-col justify-center gap-[1px] overflow-hidden shadow-sm ${aulaAqui.status_pagamento === 'Pago' ? 'bg-emerald-100 border-emerald-200 text-emerald-700' : aulaAqui.status_pagamento === 'Parcial' ? 'bg-amber-100 border-amber-200 text-amber-700' : 'bg-pink-100 border-pink-200 text-pink-700'} ${Array.isArray(aulaAqui.nome_professor) && aulaAqui.nome_professor.length === 0 ? 'ring-2 ring-red-400 ring-inset' : ''}`}>
          ...
          <div className="relative bg-white w-full max-w-lg rounded-t-[32px] sm:rounded-[32px] p-6 pb-40 max-h-[90vh] overflow-y-auto shadow-2xl animate-in slide-in-from-bottom-full duration-300" style={{ transform: `translateY(${dragOffset}px)` }}>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col"><label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 flex items-center gap-1.5"><Calendar size={14}/> {t.aulas.dataAula}</label><input type="date" {...register('data_aula', { required: true })} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-4 text-base focus:outline-none focus:border-pink-500 focus:ring-1 focus:ring-pink-500 transition-all" /></div>
                <div className="flex flex-col"><label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 flex items-center gap-1.5"><Clock size={14}/> {t.aulas.horario}</label><select {...register('horario', { required: true })} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-4 text-base font-bold text-slate-700 outline-none focus:border-pink-500 appearance-none"><option value="">{t.aulas.selecione}</option>{OPCOES_HORARIOS.map(h => <option key={h} value={h}>{h}</option>)}</select></div>
              </div>
              
              <div><label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5 block">{t.aulas.modalidade}</label><div className="grid grid-cols-2 gap-3"><label className="relative cursor-pointer"><input type="radio" value="Aula Particular" {...register('modalidade')} className="peer sr-only" /><div className="bg-slate-50 border border-slate-200 peer-checked:border-pink-500 peer-checked:bg-pink-50 rounded-xl py-4 text-center font-bold text-sm text-slate-500 peer-checked:text-pink-700 transition-all">{t.aulas.aulaParticular}</div></label><label className="relative cursor-pointer"><input type="radio" value="Aula Grupo" {...register('modalidade')} className="peer sr-only" /><div className="bg-slate-50 border border-slate-200 peer-checked:border-pink-500 peer-checked:bg-pink-50 rounded-xl py-4 text-center font-bold text-sm text-slate-500 peer-checked:text-pink-700 transition-all">{t.aulas.aulaGrupo}</div></label></div></div>
              
              <div className="relative"><label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5 flex items-center gap-1.5"><User size={13}/> {t.aulas.alunoPrincipal}</label><input type="text" autoComplete="off" placeholder={t.aulas.nomeCliente} {...register('nome_cliente', { required: true })} onFocus={() => setMostrarSugestoes(true)} onBlur={() => setTimeout(() => setMostrarSugestoes(false), 200)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-4 text-base focus:outline-none focus:border-pink-500 focus:ring-1 focus:ring-pink-500 relative z-20" />{mostrarSugestoes && sugestoesAlunos.length > 0 && nomeClienteAtual.length > 0 && (<ul className="absolute z-30 w-full mt-1 bg-white border border-slate-200 rounded-xl shadow-[0_8px_30px_rgba(0,0,0,0.12)] max-h-48 overflow-y-auto top-[76px] divide-y divide-slate-50">{sugestoesAlunos.map(aluno => (<li key={aluno.id} onClick={() => { setValue('nome_cliente', aluno.nome, { shouldValidate: true }); setMostrarSugestoes(false); }} className="px-4 py-3.5 text-sm font-bold text-slate-700 hover:bg-pink-50 hover:text-pink-700 cursor-pointer transition-colors">{aluno.nome}</li>))}</ul>)}</div>
              
              <div>
                <div className="flex items-center justify-between mb-1.5"><label className="text-xs font-bold text-slate-500 uppercase tracking-wider flex items-center gap-1.5"><GraduationCap size={13}/> {t.aulas.professores}</label><span className="text-[10px] font-bold text-slate-400 bg-slate-100 px-2 py-0.5 rounded-md">{t.aulas.opcional}</span></div>
                {listaProfessores.length === 0 ? (<p className="text-xs text-slate-400 italic">{t.aulas.nenhumProfAtivo}</p>) : (<div className="grid grid-cols-2 gap-2">{listaProfessores.map(nome => { const sel = professores.includes(nome); return (<label key={nome} className={`flex items-center gap-2 border rounded-xl px-3 py-3 cursor-pointer transition-all ${sel ? 'border-pink-500 bg-pink-50 shadow-sm' : 'bg-slate-50 border-slate-200 hover:border-slate-300'}`}><input type="checkbox" className="sr-only" checked={sel} onChange={() => toggleProfessor(nome)} /><div className={`w-4 h-4 rounded border-2 flex items-center justify-center shrink-0 transition-colors ${sel ? 'bg-pink-600 border-pink-600' : 'border-slate-300'}`}>{sel && <CheckCircle size={10} className="text-white" />}</div><span className={`text-sm font-bold truncate ${sel ? 'text-pink-700' : 'text-slate-600'}`}>{nome.split(' ')[0]}</span></label>) })}</div>)}
              </div>

              {pacotes.length > 0 && (<div><label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5 flex items-center gap-1.5"><Package size={13}/> {t.aulas.pacoteOpcional}</label><select value={pacoteSelecionado} onChange={e => handlePacoteChange(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-4 text-base focus:outline-none focus:border-pink-500 focus:ring-1 focus:ring-pink-500"><option value="">{t.aulas.nenhumPacote}</option>{pacotes.map(p => (<option key={p.id} value={p.id}>{p.nome_cliente} ({p.aulas_restantes} {t.aulas.restantes})</option>))}</select></div>)}

              {!temPacote && (
                <>
                  <div className="grid grid-cols-2 gap-3">
                    <div><label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5 flex items-center gap-1.5"><DollarSign size={13}/> {t.aulas.valorTotal}</label><input type="number" step="0.01" min="0" placeholder="0,00" {...register('valor_aula', { required: !temPacote, valueAsNumber: true })} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-4 text-base focus:outline-none focus:border-pink-500 focus:ring-1 focus:ring-pink-500" /></div>
                    <div><label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5 block">{t.aulas.pagamento}</label><select {...register('status_pagamento')} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-4 text-base focus:outline-none focus:border-pink-500 focus:ring-1 focus:ring-pink-500"><option value="Pendente">{t.aulas.pendente}</option><option value="Parcial">{t.aulas.parcial}</option><option value="Pago">{t.aulas.pagoTotal}</option></select></div>
                  </div>
                  
                  {statusPagamento === 'Parcial' && (
                    <div className="bg-sky-50 border border-sky-100 rounded-xl p-4 mt-2">
                      <label className="text-xs font-bold text-sky-800 uppercase tracking-wider mb-1.5 block">{t.aulas.valorSinal}</label>
                  ...
                  <div className="mt-2">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5 block">{t.aulas.meioPagamento}</label>
                    <select {...register('forma_pagamento')} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-4 text-base focus:outline-none focus:border-pink-500 focus:ring-1 focus:ring-pink-500">
                      <option value="">{t.aulas.naoInformado}</option>
      ...
      {aulaDetalheGrade && (
        <div className="fixed inset-0 z-[70] flex items-end justify-center sm:items-center">
          ...
          <div className="relative w-full max-w-md pb-8 px-4 animate-in slide-in-from-bottom-4 duration-300">
             <div className="flex justify-end mb-3"><button onClick={() => setAulaDetalheGrade(null)} className="w-10 h-10 bg-white/20 backdrop-blur-md border border-white/30 rounded-full flex items-center justify-center text-white hover:bg-white/30 transition-colors"><X size={20} /></button></div>
